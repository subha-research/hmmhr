...
frappe.ui.form.on("Shift Assignment Tool", {
	setup(frm) {
		hmmhr.setup_employee_filter_group(frm);
	},
	...
	refresh(frm) {
		...
		hmmhr.handle_realtime_bulk_action_notification(
			frm,
	...
	render_employees_datatable(frm, employees) {
		...
		hmmhr.render_employees_datatable(frm, columns, employees, no_data_message);
	},
	...
	bulk_assign(frm, employees) {
		...
		hmmhr.validate_mandatory_fields(frm, selected_employees);
		frappe.confirm(
	...
	process_shift_requests(frm, status) {
		...
		hmmhr.validate_mandatory_fields(frm, selected_requests, "Shift Requests");
		frappe.confirm(
