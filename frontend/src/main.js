...
const registerServiceWorker = async () => {
	window.frappePushNotification = new FrappePushNotification("hmmhr")

	if ("serviceWorker" in navigator) {
		let serviceWorkerURL = "/assets/hmmhr/frontend/sw.js"
		let config = ""
...
router.isReady().then(async () => {
	if (import.meta.env.DEV) {
		await frappeRequest({
			url: "/api/method/hmmhr.www.hmmhr.get_context_for_dev",
		}).then(async (values) => {
